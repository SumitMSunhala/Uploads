...
namespace GlobalHRMSApi.BLL
{
  public class EmployeeLogic
  {
    ...
    public bool IsMobileNumberExists(string mobileNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsMobileNumberExists(mobileNumber, employeeId));
    }
    public bool IsEmailIdExists(string emailId, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsEmailIdExists(emailId, employeeId));
    }
    public bool IsAadharCardNumberExists(string aadharCardNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsAadharCardNumberExists(aadharCardNumber, employeeId));
    }
    public bool IsAadharEnrolmentNumberExists(string aadharEnrolmentNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsAadharEnrolmentNumberExists(aadharEnrolmentNumber, employeeId));
    }
    public bool IsPANCardNumberExists(string panCardNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsPANCardNumberExists(panCardNumber, employeeId));
    }
    public bool IsElectionCardNumberExists(string electionCardNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsElectionCardNumberExists(electionCardNumber, employeeId));
    }
    public bool IsRationCardNumberExists(string rationCardNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsRationCardNumberExists(rationCardNumber, employeeId));
    }
    public bool IsNationalPopulationRegisterExists(string nationalPopulationRegister, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsNationalPopulationRegisterExists(nationalPopulationRegister, employeeId));
    }
    public bool IsDrivingLicenceNumberExists(string drivingLicenceNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsDrivingLicenceNumberExists(drivingLicenceNumber, employeeId));
    }
    public bool IsPassportNumberExists(string passportNumber, int? employeeId)
    {
      return Convert.ToBoolean(hrmsEntities.IsPassportNumberExists(passportNumber, employeeId));
    }
        ...
        employeesList = employeeDetailsResult.Where(x => x.TableName == Common.Constants.EmployeePersonalDetails)
            .Select(x => new GlobalHRMSApi.Models.EmployeePersonalDetailsList
            {
              ...
              EmployeeCompanyDetailsList = employeeDetailsResult.Where(z => z.TableName == Common.Constants.EmployeeCompanyMaster && z.EmployeeId == x.EmployeeId).Select(z => new EmployeeCompanyDetails
              {
                ...
                GradeId = Convert.ToInt32(z.GradeId),
                DepartmentId = Convert.ToInt32(z.DepartmentId),
                UnitId = Convert.ToInt32(z.UnitId),
